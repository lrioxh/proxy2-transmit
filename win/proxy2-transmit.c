...
int main()
{
    ...
    do {
        ...
        while (1)
        {
            ...
            setsockopt(midClntSocket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));//超时返回-1
            //向服务器发出连接请求
            if (
            ...
            while (1) {
                ...
                //ioctlsocket(midClntSocket, FIONBIO, &nonBlockingMode);
                while (1) {
                    //from server
                    //Sleep(5);
                ...
                //ioctlsocket(midClntSocket, FIONBIO, &blockingMode);
                //break;
            }
            closesocket(midSrvConn);
            closesocket(midClntSocket);//关闭
